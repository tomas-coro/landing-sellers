-- Landing Sellers
-- Registra un pagamento incassato insieme al relativo snapshot economico.
--
-- La funzione esistente registra_pagamento_vendita continua a gestire
-- pagamenti/rate e rimane disponibile per compatibilità e rate previste.
--
-- Questa funzione la riutilizza e aggiunge, nella stessa transazione:
-- - pagamento_calcoli
-- - pagamento_partecipanti
...
begin
  ...
  if exists (
    ...
    raise exception 'Partecipante non appartenente alla vendita';
  end if;
  ...
  if exists (
    ...
    where pc.pagamento_id = v_pagamento_id
  ) then
  ...
  values (
    ...
    v_margine,
    coalesce(
      (p_calcolo ->> 'percentuale_tasse')::numeric,
      0
    ),
    coalesce(
      (p_calcolo ->> 'percentuale_fatturata_admin')::numeric,
      0
    ),
    v_importo_tasse,
    v_netto,
    coalesce(
      p_calcolo -> 'costi_snapshot',
      '[]'::jsonb
    )
  );
  ...
  select
    ...
    coalesce(
      nullif(elemento ->> 'modalita_fatturazione', ''),
      'nessuna'
    ),
    coalesce(
      (elemento ->> 'importo_fatturato')::numeric,
      0
    ),
    coalesce(
      (elemento ->> 'quota_base')::numeric,
      0
    ),
    coalesce(
      (elemento ->> 'quota_teorica')::numeric,
      0
    ),
    coalesce(
      (elemento ->> 'percentuale_riduzione')::numeric,
      0
    ),
    coalesce(
      (elemento ->> 'importo_riduzione')::numeric,
      0
    ),
    coalesce(
      (elemento ->> 'bonus_admin')::numeric,
      0
    ),
    coalesce(
      (elemento ->> 'quota_calcolata')::numeric,
      0
    ),
    coalesce(
      (elemento ->> 'quota_effettiva')::numeric,
      0
    ),
    coalesce(
